...
# ===== VALORES DE CALIBRAÇÃO E LIMITES =====
# [centro, min, max] - ajuste após calibrar
CAL = {
    6: [500, 139, 861],  # base
    5: [500, 129, 868],  # ombro
    4: [500, 139, 865],  # cotovelo
    3: [500, 120, 861],  # punho_rot
    2: [500, 139, 861],  # punho_incl
    1: [500, 139, 861]   # garra
}

# Limites [min_ang, max_ang]
LIM = {
    6: [-90, 90],
    5: [0, 180],
    4: [-90, 90],
...
def angle_to_servo(angle, idx):
    # Inverte o ângulo para juntas específicas
    if idx == 2:
        angle = -angle
    ...
    if idx < len(config.juntas_controle):
        ...
        # Usa limites calibrados se disponíveis
        if servo_id in config.servo_limits:
            min_servo, max_servo = config.servo_limits[servo_id]
        elif servo_id in CAL:
            centro, min_servo, max_servo = CAL[servo_id]
        else:
            min_servo, max_servo = 139, 861  # fallback
        
        # Usa limites angulares se disponíveis
        if servo_id in LIM:
            min_angle, max_angle = LIM[servo_id]
        else:
            # Fallback para limites genéricos
...
def configurar_padrao():
    ...
    # Configura as juntas com base nos limites definidos
    config.juntas_controle = [
        ["Base", LIM[6][0], LIM[6][1], 0, 6],
        ["Ombro", LIM[5][0], LIM[5][1], 90, 5],
        ["Cotovelo", LIM[4][0], LIM[4][1], 0, 4],
        ["Punho Rot.", LIM[3][0], LIM[3][1], 0, 3],
        ["Punho Incl.", LIM[2][0], LIM[2][1], 0, 2],
        ["Garra", LIM[1][0], LIM[1][1], 0, 1]
    ]
    ...
    # Carrega os limites dos servos a partir do CAL
    config.servo_limits = {}
    for servo_id, valores in CAL.items():
        if len(valores) >= 3:
            centro, min_servo, max_servo = valores
            config.servo_limits[servo_id] = [min_servo, max_servo]
...
def criar_sliders():
    ...
    for i, (label, lo, hi, default, canal) in enumerate(config.juntas_controle):
        ...
        # Mostra informações de calibração
        cal_info = ""
        if canal in CAL:
            centro, min_s, max_s = CAL[canal]
            cal_info = f"{min_s}-{max_s}"
        ...
        tk.Label(frame_sliders, text=cal_info, font=FONT_SMALL,
                 bg=SURFACE, fg=ORANGE, width=10, anchor="center"
...
def verificar_limites(x, y, z):
    ...
    # Verifica limites angulares baseados em LIM
    for i, sv in enumerate(servo_vars):
        if i < len(config.juntas_controle):
            canal = config.juntas_controle[i][4]
            if canal in LIM:
                min_angle, max_angle = LIM[canal]
                angulo_atual = sv.get()
                if angulo_atual < min_angle or angulo_atual > max_angle:
                    return False, f"Junta {config.juntas_controle[i][0]} fora do limite!\n" \
                                 f"Limite: [{min_angle}°, {max_angle}°]\n" \
                                 f"Atual: {angulo_atual}°"
